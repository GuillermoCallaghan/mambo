...
uint64_t allocate_aibi_slot(dbm_thread *thread_data, uint32_t **o_write_p) {
  ...
  uintptr_t *clr = (uintptr_t *) aibi_slot;
  clr[0] = 0;
  clr[1] = 0;
...
void a64_branch_jump_cond(dbm_thread *thread_data, uint32_t **o_write_p, int basic_block,
  ...
  *write_p = NOP_INSTRUCTION;
  ...
  generate_address(&write_p, x0, LR);
  a64_b_helper(write_p, thread_data->dispatcher_addr);
  write_p++;
...
void a64_inline_hash_lookup(dbm_thread *thread_data, int basic_block, uint32_t **o_write_p,
   ...
   *     branch:           pop   x2, [sp], #16                   ***
   *                       br    x0
   *     go_to_dispatcher: mov   x0, x1
   *                       mov   x1, #basic_block
   *                       pop   x2, [sp], #16                   ***
   *                       b     #dispatcher
   *
   * *** if rn is x0 or (blr lr)
   * ### for blr
   */
...
size_t scan_a64(dbm_thread *thread_data, uint32_t *read_address,
  ...
  uint32_t const *start_address = write_p;
...
#if defined(RAIBI) || defined(AIBI)
        uint32_t const ihlu_size = 33 * A64_INSTRUCTION;
#else
        uint32_t const ihlu_size = 22 * A64_INSTRUCTION;
#endif
